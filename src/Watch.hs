...
eventFile (Added fp _ _) = fp
eventFile (Modified fp _ _) = fp
eventFile (Removed fp _ _) = fp
eventFile (Unknown fp _ _) = fp
...
wEvalPred (Added _ _ _) PredAdded = True
wEvalPred (Modified _ _ _) PredModified = True
wEvalPred (Removed _ _ _) PredRemoved = True
...
readEvent :: (MonadUnliftIO m, MonadIO m) => Chan Event -> m Event
readEvent chan = do
...
readEventNewerThan :: (MonadUnliftIO m, MonadIO m) => Chan Event -> UTCTime -> m ()
